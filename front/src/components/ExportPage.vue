<template>
  <main class="p-0">
    <b-container>
      <div class="row justify-content-between align-items-center main-row">
        <div class="col-fluid p-md-0 px-3">
          <button
            class="btn d-flex export cta py-md-3 px-md-4"
            v-if="!isResultSent && !isDone"
            @click="makeExport"
          >
            {{ $t('Export') }}
          </button>
          ...
          <div
            class="wait d-flex align-items-start"
            v-if="isResultSent && !isDone"
          >
          ...
          <div class="load d-flex align-items-center" v-if="isDone">
            <a
              class="message-title d-md-block px-3"
              target="_blank"
              :href="'/media/exports/' + exportId + '.pdf'"
            >
            ...
            <button
              type="button"
              class="btn btn-outline-red pt-1 pb-1 disabled-message"
              @click="exportAgain"
            >
              {{ $t('Make export again') }}
            </button>
          ...
          <div class="alert-cont d-flex align-items-start" v-if="hasError">
            ...
            <div>
              ...
              <div class="message-subtitle">
                {{ $t('Something') }} <br class="d-md-none" />
                {{ $t('went wrong') }}
              </div>
            </div>
          </div>
        ...
        <b-button
          ...
          class="btn float-right b-btn-outline-blue pt-1 pb-1 d-none d-md-block"
          @click="createURL($t('Success!'), $t('Link copied to the clipboard'))"
        >
          ...
          <div data-spy="scroll" data-target="#list" data-offset="0" class="sc">
            ...
            >
              <div class="col-12 col-md-8">
                ...
                <div
                  class="card-group"
                  v-for="(req, index_req) in cat.requirements"
                >
                  <div class="d-flex align-items-flex-start">
                    <b-form-checkbox
                      :key="`selectedrequirements-${index_req}`"
                      :model="getSelectedCategoriesIds[cat.id]"
                      @change="turnSwitcher($event, cat.id, req.id)"
                      :value="req.id"
                    />
                    <b-form-checkbox
                      :key="`requirements-${index_req}`"
                      :model="getSelectedCategoriesIds[cat.id]"
                      @change="turnCheckbox($event, cat.id, req.id)"
                      :value="req.id"
                      switch
                    >
                      <span
                        v-b-popover.hover="$t('Mark requirement as passed')"
                        >{{ req.title }}</span
                      >
                    </b-form-checkbox>
                  </div>
    ...
    >
      ...
      <div
        class="col-12 security-card"
        v-if="JSON.stringify(currentRequirement) !== '{}'"
      >
        ...
        <p class="p-text" v-if="currentRequirement.text">
          <vue-simple-markdown
            :source="currentRequirement.text"
          ></vue-simple-markdown>
        </p>
        ...
        <div v-if="currentRequirement.test_cases.length">
          ...
          <ul class="card-list p-0 pl-3">
            <li
              class="card-list--item pl-2"
              v-for="(test_case,
              test_case_cat) in currentRequirement.test_cases"
            >
...
export default {
  ...
  created() {
    ...
    this.getSelectedCategoriesIds.map(i => {
      this.selectedReq[i] = []
      this.selectedPassed[i] = []
    })
  },
  methods: {
    turnCheckbox: function($event, catId, reqId) {
      if ($event === false) {
        this.selectedPassed[catId] = this.selectedPassed[catId].filter(r => {
          reqId !== r
        })
      } else {
        this.selectedPassed[catId].push(reqId)
      }
    },
    turnSwitcher: function($event, catId, reqId) {
      if ($event === false) {
        this.selectedReq[catId] = this.selectedReq[catId].filter(r => {
          reqId !== r
        })
      } else {
    ...
    makeExport: function() {
      ...
      for (const catId of Object.keys(this.selectedReq))
        postData.push({
          category_id: catId,
          requirements_ids: this.selectedReq[catId],
          passed_requirements_ids: this.selectedPassed[catId],
        })
